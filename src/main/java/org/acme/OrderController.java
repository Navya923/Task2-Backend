...
public class OrderController {
    ...
    public List<Cart> getCarts() {
        System.out.println("fetching cart list for order");
    ...
    public Response approveCart(@PathParam("id") long id) {
        ...
        cart.setStatus(Status.ORDER_APPROVAL);
        cart.persist();
    ...
    public Response rejectCart(@PathParam("id") long id) {
        Cart cart = Cart.findById(id);
        cart.setStatus(Status.ORDER_REJECT);
        cart.persist();
        return Response.status(Response.Status.OK).build();
    }
    ...
    public List<Cart> GetRejectedCarts() {
        System.out.println("fetching rejected cart list for order");
        List<Cart> carts = cartRepository.getCarts(Arrays.asList(Status.ORDER_REJECT));
        System.out.println(carts);
        return carts;
    }
